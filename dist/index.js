...
// ─── Database ────────────────────────────────────────────
const pool = new pg_1.Pool({ connectionString: DATABASE_URL, max: 20 });
...
async function lookupDomain(host) {
    ...
    if (cached && Date.now() - cached.ts < CACHE_TTL)
        return cached.data;
    const { rows } = await pool.query(`SELECT id, workspace_id, domain, verified, supports_subdomains FROM domains WHERE domain = $1 AND verified = true LIMIT 1`, [host]);
    const record = rows[0] || null;
    domainCache.set(host, { data: record, ts: Date.now() });
    return record;
}
async function lookupParentDomain(parentDomain) {
    const cached = domainCache.get(`parent:${parentDomain}`);
    if (cached && Date.now() - cached.ts < CACHE_TTL)
        return cached.data;
    const { rows } = await pool.query(`SELECT id, workspace_id, domain, verified, supports_subdomains FROM domains WHERE domain = $1 AND verified = true AND supports_subdomains = true LIMIT 1`, [parentDomain]);
    const record = rows[0] || null;
...
function isBot(req) {
    ...
    if (REAL_BROWSER_PATTERN.test(ua))
        return false;
...
function extractSubdomain(host) {
    ...
    if (parts.length <= 2)
        return { subdomain: null, parent: host };
...
app.use((0, pino_http_1.default)({ logger, autoLogging: { ignore: (req) => req.url === "/health" } }));
// Health check
